...
function UnderwaterParticles({ count = 7000 }) {
  ...
  useFrame((state) => {
    if (particlesRef.current) {
      particlesRef.current.material.uniforms.uTime.value =
  ...
  const particleMaterial = useMemo(
    () =>
      new THREE.ShaderMaterial({
        ...
        vertexShader: `uniform float uTime; varying float vAlpha; void main(){vec3 pos=position;pos.x+=sin(uTime*.5+position.y*.5)*.3;pos.y+=mod(uTime*.4+position.y*.1,35.)-30.;pos.z+=cos(uTime*.3+position.x*.3)*.2;vAlpha=.3+sin(uTime*2.+position.x*10.)*.2;vec4 mvPosition=modelViewMatrix*vec4(pos,1.);gl_PointSize=2.5*(1.-mvPosition.z*.05);gl_Position=projectionMatrix*mvPosition;}`,
        fragmentShader: `varying float vAlpha; void main(){float dist=length(gl_PointCoord-vec2(.5));if(dist>.5)discard;gl_FragColor=vec4(.8,.9,1.,(1.-dist*2.)*vAlpha*.4);}`,
      }),
    []
  );
  return (
  ...
  useFrame((state) => {
    if (plantRef.current) {
      plantRef.current.children.forEach((leaf, i) => {
        leaf.rotation.z =
          Math.sin(state.clock.elapsedTime * 0.8 + i * 0.5) * 0.2;
      });
    }
  });
  return (
...
function ServiceCard({
  ...
  useFrame((state) => {
    if (cardRef.current) {
      const floatAmount =
        Math.sin(state.clock.elapsedTime * 0.4 + position[0]) * 0.15;
      cardRef.current.position.y = position[1] + floatAmount;

      if (!oblique) {
        cardRef.current.rotation.x =
          Math.sin(state.clock.elapsedTime * 0.3 + position[0]) * 0.06;
        cardRef.current.rotation.z =
          Math.sin(state.clock.elapsedTime * 0.2 + position[0]) * 0.03;
      }
    }
  ...
  return (
    <group
      ref={cardRef}
      position={position}
      rotation={oblique ? [0, Math.PI / 6, 0] : [0, 0, 0]}
    >
      <mesh position={[0.05, -0.05, -0.15]} castShadow>
        <boxGeometry args={[3.3, 3.9, 0.3]} />
        <meshStandardMaterial
          color="#0a2a3a"
          transparent
          opacity={opacity * 0.5}
      ...
      <group ref={frontTextRef} position={[0, 0, 0.25]}>
      ...
      <mesh position={[0, 1.2, -0.23]} rotation={[0, Math.PI, 0]}>
        <circleGeometry args={[0.45, 32]} />
        <meshStandardMaterial
          color="#4a9aaa"
          transparent
...
function UnderwaterLighting() {
  return (
    <>
      <ambientLight intensity={0.35} color="#5090a0" />
      <directionalLight
        position={[0, 10, 0]}
        intensity={0.7}
        color="#a0d8f0"
    ...
    >
      ...
      <div
        style={{
          ...
          fontSize: ".9rem",
          pointerEvents: "none",
          background: "rgba(42, 106, 122, 0.6)",
          padding: "1rem 1.5rem",
          borderRadius: "20px",
